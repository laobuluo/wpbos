...
if (version_compare(PHP_VERSION, '7.4', '<')) {
    register_activation_hook(__FILE__, function () {
        ...
        wp_die(
            '<h1>插件激活失败</h1><p><strong>WPBOS</strong> 需要 PHP 7.4 或更高版本，当前版本：' . esc_html(PHP_VERSION) . '。请升级 PHP 后再激活本插件。</p>',
            '插件激活失败',
            array('back_link' => true)
        );
    });
    add_action('admin_notices', function () {
        echo '<div class="notice notice-error"><p><strong>WPBOS</strong> 需要 PHP 7.4 或更高版本，当前版本：' . esc_html(PHP_VERSION) . '。请升级 PHP 或停用本插件。</p></div>';
    });
    return;
}

if (!class_exists('WPBOS')) {
    class WPBOS {

        private $option_name     = 'WPBOS_options';               // 插件参数保存名称
        private $menu_title      = 'BOS对象存储设置';               // 设置菜单的菜单名
        private $page_title      = 'BOS对象存储设置';               // 设置菜单的页面title
        private $capability      = 'manage_options';              // 设置页面管理所需权限
        private $version         = '3.1';                         // 插件数据版本， 每次修改应与上方的Version值相同
        private $setting_notices = [
            'update_success' => 'WPBOS插件设置保存完毕!!!',         // post数据保存成功时提示内容
            'update_failed'  => 'WPBOS插件设置更新失败!!!',         // 失败时提示
        ];
        ...
        function __construct() {
            ...
            if (substr_count($request_uri, '/update.php') <= 0) {
        ...
        private function constants() {
            ...
            $this->object_storage = new BosStorageObjectApi($this->options);  // option更新后，若变动了参数，则Api实例重新创建，目前只有setting中会触发
        ...
        public function setting_page() {
            ...
            if ($this->options && isset($_GET['_wpnonce']) && wp_verify_nonce($_GET['_wpnonce']) && !empty($_POST)) {
                if($_POST['type'] == 'info_set') {
                    ...
                    $this->options['secretAccessKey'] = isset($_POST['secretAccessKey']) ? sanitize_text_field(trim(stripslashes($_POST['secretAccessKey']))) : '';
                    $this->options['protocol'] = isset($_POST['protocol']) ? sanitize_text_field(trim(stripslashes($_POST['protocol']))) : 'https';
                    ...
                    $this->object_storage = new BosStorageObjectApi($this->options);
